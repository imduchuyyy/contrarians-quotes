...
const Header = ({ title, visible, visibleBackButton, showMenu, onClickBurger }: HeaderProps) => {
    ...
    return (
        <>
            ...
            >
                <div
                    className={`relative flex items-center justify-between h-24 w-full mx-auto px-10 lg:px-6 md:h-16 md:px-4 md:rounded-2xl md:transition-shadow ${
                        showMenu
                ...
                >
                    {visibleBackButton ? (
                        <button
                            className="group inline-flex items-center mr-auto text-xl md:hidden"
                            onClick={() => router.back()}
                        >
                    ...
                    <div className="absolute left-1/2 transform -translate-x-1/2 text-h3 md:text-h4 sm:text-h5 text-center font-bold">
                        Your Contrarian types
                    </div>

                    <button
                        className="absolute right-10 group flex items-center h-12 px-4 rounded-xl transition-colors hover:bg-theme-on-surface-2 md:hover:bg-transparent lg:right-6"
