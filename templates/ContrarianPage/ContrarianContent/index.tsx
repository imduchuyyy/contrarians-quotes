...
const pickTrait = (arr: Trait[], numItems: number): Trait[] => {
    const shuffled = arr?.slice().sort(() => 0.5 - Math.random());
    return shuffled?.slice(0, numItems);
};

const ContrarianContent = ({ data, contrarianId }: ContrarianContentProps) => {
    ...
    const randomAttributes: Trait[] = attributes ? attributes : [];
    const traits = contrariansTraits.filter((trait: Traits) => {
        return randomAttributes.some((attribute: Trait) => attribute.value === trait.Trait);
    }).slice(0, 3);

    return (
        <div className="flex sm:flex-col-reverse flex-row lg:space-x-2 lg:space-y-0 space-y-3">
            {/* Text Content */}
            ...
            <div className="flex-shrink-0 w-1/2 px-10 sm:w-full md:py-10 sm:py-0 sm:mt-10 mt-5 flex flex-col items-center justify-end sm:items-center">
                ...
                <ShareButton traits={traits} contrarianId={contrarianId} />
            </div>
        </div>
    );
};
